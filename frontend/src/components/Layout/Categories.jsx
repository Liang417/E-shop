        ...
        <div className="grid grid-cols-1 gap-[5px] md:grid-cols-2 md:gap-[10px] lg:grid-cols-4 lg:gap-[20px] xl:grid-cols-5 xl:gap-[30px]">
          {categoriesData &&
            categoriesData.map((item) => {
              ...
              return (
                <div
                  className="w-full h-[100px] flex items-center justify-between cursor-pointer overflow-hidden hover:text-green-500"
                  key={item.id}
                  onClick={() => handleSubmit(item)}
                >
                  <h5 className={`text-[18px] leading-[1.3]`}>{item.title}</h5>
                  <img src={item.image_Url} className="w-[120px] object-cover" alt="" />
                </div>
              );
            })}
        </div>
